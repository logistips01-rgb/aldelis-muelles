...
// L1/L2/L3: 16.000€/mes ÷ diasLaborables ÷ 1440 min/día (24 h)
...
function recalcLanzCosteMin() {
  ...
  LANZ_COSTE_MIN = {
    1: LANZ_MENSUAL / (d * LANZ_MIN_DIA[1]),
    2: LANZ_MENSUAL / (d * LANZ_MIN_DIA[2]),
    3: LANZ_MENSUAL / (d * LANZ_MIN_DIA[3]),
    4: LANZ4_HORA / 60
  };
}
recalcLanzCosteMin();
...
function abrirPtDetalle(id) {
  ...
  if (esEnvase) {
    lineasEl.innerHTML = "<div class='pt-admin-tit'>Contenido (" + lineas.length + " referencia" + (lineas.length === 1 ? "" : "s") + ")</div>" +
      ...
      lineas.map(l => {
        // Europool (y Logifruit en Txt) va remontado: la cantidad son
        // unidades reales (el doble de lo que ocupa en huecos de camion),
        // asi que se aclara aparte para que cuadre a simple vista con el
        // total de "Palets del pedido".
        const cat = CATALOGO_ENVASES_AVITRANS.find(c => c.ref === l.ref);
        const comoEuropool = cat && (cat.tipo === "europool" || (cat.desc.includes("LOGIFRUIT") && p.almacen === "txt"));
        const huecos = comoEuropool ? " (" + Math.ceil((l.cantidad || 0) / 2) + " hueco" + (Math.ceil((l.cantidad || 0) / 2) === 1 ? "" : "s") + " de camión)" : "";
...
function probarEscrituraBuzon() {
  const buzon = document.getElementById("cfg-buzon-prueba").value.trim();
  const cont = document.getElementById("cfg-buzon-escritura-resultado");
  if (!buzon) { cont.style.color = "#D41F3A"; cont.textContent = "Pon primero el buzón de arriba."; return; }
  cont.style.color = "";
  cont.textContent = "Probando...";
  firebase.functions().httpsCallable("probarEscrituraBuzon")({ buzon })
    .then(res => {
      if (!res.data || !res.data.ok) {
        cont.style.color = "#D41F3A";
        cont.textContent = (res.data && res.data.error) || "No se pudo probar la lectura/escritura.";
        return;
      }
      cont.style.color = res.data.aviso ? "#F59E0B" : "#1D9E75";
      cont.textContent = res.data.aviso ||
        ("Funciona correctamente sobre \"" + res.data.asunto + "\" — el permiso de lectura/escritura está activo.");
    })
    .catch(e => {
      cont.style.color = "#D41F3A";
      cont.textContent = "Error: " + e.message;
